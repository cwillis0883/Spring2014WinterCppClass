...
void check_equal_wcompare(const wchar_t* const expected, const wchar_t* const actual, TestResult& result, char* file, int line);

template <typename T, typename U> void check_equal_wcompare(T expected, U actual, TestResult& result, char* file, int line)
{
    if (! ((expected) == (actual))) {
...
{\
	check_equal_wcompare(expected, actual, rEsUlT_, char* file, int line, __FILE__, __LINE__); \
}
...
template <typename T, typename U> void check_equal_compare(T expected, U actual, TestResult& result, char* file, int line)
{
    if (! ((expected) == (actual))) {
        std::ostringstream message;
